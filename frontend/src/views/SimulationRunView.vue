...
// Read maxRounds from query params at init time so child components receive it immediately
  ...
  try {
    ...
    if (envStatusRes.success && envStatusRes.data?.env_alive) {
      ...
      try {
        ...
        addLog('Failed to close simulation env, attempting force stop...')
        try {
      ...
      // Env not running, check if process needs to be stopped
...
const loadGraph = async (graphId) => {
  // During simulation, auto-refresh skips full-screen loading to prevent flickering.
  // Manual refresh or initial load shows loading state.
  if (!isSimulating.value) {
...
onMounted(() => {
  ...
  // Log maxRounds config (value already read from query params at init)
  if (maxRounds.value) {
