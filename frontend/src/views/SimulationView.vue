    ...
    if (envStatusRes.success && envStatusRes.data?.env_alive) {
      ...
      try {
        ...
        if (closeRes.success) {
          ...
          // If graceful close fails, attempt force stop
          await forceStopSimulation()
        }
      } catch (closeErr) {
        addLog(`Exception closing simulation env: ${closeErr.message}`)
        // If graceful close throws, attempt force stop
        await forceStopSimulation()
      }
    } else {
      // Env not running, but process may still be active — check simulation status
